...
public void setup() 
{
    ...
    plotResp.setPos(0,(totalPlotsHeight/3+70));
   ...
   cp5.addButton("Exit")
    ...
    tglRecord = cp5.addToggle("record")
     ...
     .setMode(ControlP5.SWITCH);
      ...
      if(!System.getProperty("os.arch").contains("arm"))
      {     
          cp5.addScrollableList("Select HealthyPi port")
             .setPosition(300, 5)
             .setSize(150, 100)
             .setColorBackground(color(255,255,255))
             .setColorLabel(color(0))
             ...
             .addCallback(new CallbackListener() 
             {
                public void controlEvent(CallbackEvent event) 
                {
                  if (event.getAction() == ControlP5.ACTION_RELEASED) 
                  {
                    ...
                    System.out.println(globalPortName);
                  }
                }
             } 
           );    
           ...
           //cp5.addScrollableList("Select Arduino port")
           //  .setPosition(600, 5)
           //  .setSize(150, 100)
           //  .setColorBackground(color(255,255,255))
           //  .setColorLabel(color(0))
      ...
      .setPosition(width-600,(totalPlotsHeight/3+10))
      .setColorValue(color(255,255,255))
      ...
      lblGSR = cp5.addTextlabel("lblGSR")
      .setText("GSR: --- mV")
      .setPosition(width-550,height-100)
      .setColorValue(color(255,255,255))
      .setFont(createFont("verdana",14));
      
      lblHelp = cp5.addTextlabel("lblHelp")
      .setText("Analyzing data.")
      .setPosition(width-550,height-75)
      .setColorValue(color(255,255,255))
      .setFont(createFont("verdana",14));
      
      lblResults = cp5.addTextlabel("lblResults")
      .setText("Please rest until the program is ready.")
      .setPosition(width-550,height-50)
      .setColorValue(color(255,255,255))
      .setFont(createFont("verdana",14));
    ...
    {  
        ...
        lblRR.setPosition(width-220,(totalPlotsHeight/3+totalPlotsHeight/3+10+heightHeader))
        .setFont(createFont("verdana",20));
        ...
        if(usbFiles.length<=0)
          ...
          else
          {
            ...
            try
            {
              ...
              bufferedWriter.newLine();
              bufferedWriter.write("Format: ECG, PPG, Respiration, Temperature, Heartrate, SpO2, Respiration Rate, GSR");
              bufferedWriter.newLine();
              startSerial("/dev/ttyAMA0",115200);
            }
            catch(Exception e)
...
double sd_RESP;
double sd_GSR;
ArrayList<Double> first_min_HR = new ArrayList<Double>();
ArrayList<Double> first_min_RESP = new ArrayList<Double>();
ArrayList<Double> first_min_GSR = new ArrayList<Double>();
...
void classify() {
  try {
    count++;
    boolean calculateStat = false;
    if(first_min_GSR.size() < 75000) {
      if(global_hr > 40)
        first_min_HR.add((double)global_hr);
      if(global_rr > 0)
        first_min_RESP.add((double)global_rr);
      if(GSR > 0)
        first_min_GSR.add(GSR);
    }
    else {
      if (stats) {
        avg_HR = averageValue(first_min_HR);
        avg_RESP = averageValue(first_min_RESP);
        avg_GSR = averageValue(first_min_GSR);
        
        sd_HR = sdValue(first_min_HR, avg_HR);
        sd_RESP = sdValue(first_min_RESP, avg_RESP);
        sd_GSR = sdValue(first_min_GSR, avg_GSR);
        lblHelp.setText("Program ready. Testing can begin.");
        stats = false;
      }
      else {}
    }
    if(count > 7500 && !stats) {
      ...
      String Url = "http://127.0.0.1:5000//PredictStress";
      PostRequest post = new PostRequest(Url);
      post.addData("HR", String.valueOf((global_hr - avg_HR) / sd_HR));
      post.addData("RESP", String.valueOf((global_rr - avg_RESP) / sd_RESP));
      post.addData("GSR", String.valueOf((GSR - avg_GSR) / sd_GSR));
      post.send();
      String responseStr = post.getContent();
      System.out.println("Response: " + responseStr);
      
      lblResults.setText(responseStr);
    ...
    Scanner myReader = new Scanner(output);
     GSR = 0.0;
      while (myReader.hasNextLine()) {
        ...
        try {
          GSR = Double.parseDouble(data) * (5.0 / 1023.0);
        }
        catch(Throwable e) {
        }
      }
    ...
    } else  //All  and data received
    {
      ...
      {     
        ...
        int global_RespirationRate = (int) (CES_Pkt_Data_Counter[14]);
         ...
         global_hr = (CES_Pkt_Data_Counter[16]);
         global_rr = (CES_Pkt_Data_Counter[14]);
        ...
        if (logging == true)
        {
          try 
          {
            ...
            bufferedWriter.write(ecg+","+spo2_red+","+resp+","+Temp_Value+","+global_HeartRate+","+global_spo2+","+global_RespirationRate+","+GSR);
            bufferedWriter.newLine();
          }
          catch(IOException e) 
          {
...
double averageValue(ArrayList<Double> data) {
  double avg = 0;
  
  for(int t = 1; t < data.size(); t++) {
    avg += (data.get(t) - avg) / t;
  }
  return avg;
}

double sdValue(ArrayList<Double> data, double avg) {
  ArrayList<Double> a = new ArrayList<Double>();
  double sd = 0;
  
  for(int i = 0; i < data.size(); i++) {
    a.add(Math.pow((data.get(i) - avg), 2));
  }
  double avg_a = averageValue(a);
  sd = Math.pow(avg_a / data.size(), .5);
